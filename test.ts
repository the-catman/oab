...
/** Recursively gets the keys of any particular object. */
function getKeys(obj: Record<string, any>): string[] {
    const keys = new Set<string>();

    function collectKeys(value: any): void {
...
// Some sample data automatically generated by yours truly (yeah I couldn't be bothered to type it out).
const sampleData: OABDATA = {
    "user": {
        "id": 1,
        "name": "John Doe",
...
function check(lookup?: Lookup) {
    ...
    if (JSONdata === JSON.stringify(sanityCheck)) {
        ...
        throw new Error("Sanity check: something went wrong!");
    }
    ...
    let reader = new Reader(new Uint8Array(), { lookup });
    let writer = new Writer({ lookup });

    let date = Date.now();
    for (let i = 0; i < iters; i++) {
        let someData = reader.reset(writer.reset().data(sampleData).out()).data();
        (someData as any).test = 1;
    }
